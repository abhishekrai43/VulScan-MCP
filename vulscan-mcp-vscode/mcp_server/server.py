...
After running the vulnerability scan, return only the markdown report generated by the server.
Do not summarize, interpret, or modify the report.
Do not call any additional tools or functions after the scan.
Do not generate your own answers or remediation steps—use only the server's output.
The markdown report is the final and only output.
This ensures all models and integrations give the same response.
"""
